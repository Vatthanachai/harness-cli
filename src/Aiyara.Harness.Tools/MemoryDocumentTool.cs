...
/// <summary>
/// Lets the model create or update the workspace's <c>MEMORY.md</c> directly, instead of needing to
...
/// </summary>
public class MemoryDocumentTool : BaseTool
{
    public MemoryDocumentTool()
    {
        Type = "function";
        Function = new Function
        {
            Name = "write_memory_document",
            Description = $"Creates or overwrites {MemoryDocument.FileName} at the workspace root - a project " +
                          "context file that gets loaded into the system prompt at the start of every future " +
                          "session, for facts, decisions and preferences worth remembering across sessions " +
                          "(e.g. 'user prefers tabs over spaces'). Read the file first with open_file if you " +
                          "want to add to it rather than replace it, since this overwrites the whole file.",
            Parameters = new Parameters
            {
                Properties = new Dictionary<string, Property>
                {
                    {
                        "content",
                        new Property { Type = "string", Description = $"Markdown content to write to {MemoryDocument.FileName}." }
                    }
                },
