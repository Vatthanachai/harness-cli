...
/// Stored at <c>.aiyara/skills/&lt;name&gt;/SKILL.md</c> in the workspace via <see cref="SkillStore"/>.
...
public class WriteSkillTool : BaseTool
{
    public WriteSkillTool()
    {
        ...
        {
            Name = "write_skill",
            Description = "Creates or updates a reusable skill at .aiyara/skills/<name>/SKILL.md in the " +
                          "workspace, so it can be loaded later via use_skill. Create one once you've worked out " +
                          "a repeatable approach worth reusing across sessions - not for a one-off task.",
    ...
    protected override object? Execute(IDictionary<string, object?>? args)
    {
        ...
        var overwriting = SkillStore.Exists(name);

        var approved = ActionConsent.Confirm("write_skill",
            "skill file",
            $"{(overwriting ? "update" : "create")} skill '{name}'");
        ...
        SkillStore.Save(name, description ?? "", content ?? "");

        return $"{(overwriting ? "Updated" : "Created")} skill '{name}' at {SkillStore.SkillFilePath(name)}.";
    }
}
