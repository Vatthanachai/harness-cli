...
/// <summary>
/// Lets the model create or update the workspace's <c>TOOLS.md</c> directly, instead of needing to
...
/// </summary>
public class ToolsDocumentTool : BaseTool
{
    public ToolsDocumentTool()
    {
        Type = "function";
        Function = new Function
        {
            Name = "write_tools_document",
            Description = $"Creates or overwrites {ToolsDocument.FileName} at the workspace root - a project " +
                          "context file that gets loaded into the system prompt at the start of every future " +
                          "session, describing this project's conventions for using tools (e.g. 'run dotnet " +
                          "build after editing C# files', 'never run destructive git commands').",
            Parameters = new Parameters
            {
                Properties = new Dictionary<string, Property>
                {
                    {
                        "content",
                        new Property { Type = "string", Description = $"Markdown content to write to {ToolsDocument.FileName}." }
                    }
                },
